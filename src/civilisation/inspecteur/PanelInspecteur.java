...
public class PanelInspecteur extends JPanel{
	...
	public PanelInspecteur()
	{
        ...
        String[] entetes = {I18nList.CheckLang("Variable"), I18nList.CheckLang("Valeur")};
...
//			donnees[0][1] = h.hashCode();
			...
			for (int i = var ; i < Configuration.getSchemaCognitifEnCourEdition().getAttributesNames().size() + var ; i++) {
				donnees[i][0] = "(Attribute) "+Configuration.getSchemaCognitifEnCourEdition().getAttributesNames().get(i - var);
				donnees[i][1] = h.getAttr().get(Configuration.getSchemaCognitifEnCourEdition().getAttributesNames().get(i - var));
			}
	...
	private Turtle select(int id) {
		if (World.getInstance().getTick() > 2 && World.getInstance().getTurtlesWithRoles("Humain").size() > id) return (Humain) World.getInstance().getTurtlesWithRoles("Humain").get(id);
		return null;
	}
