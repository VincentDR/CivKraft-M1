...
public class PanelEditionModifPoidsLI extends JPanel implements PropertyChangeListener, ChangeListener{
	...
	public static final int valeurMinJSpinner = -10;
	public static final int valeurMaxJSpinner = 10;
	...
	public PanelEditionModifPoidsLI(PanelModifPoidsLI parent, GCogniton gC, GPlan gP, LienPlan lien)
	{
		...
		this.valeurMin = parent.poidsMaxLI*(-1);
		this.valeurMax = parent.poidsMaxLI;
		this.pas = parent.pasModifPoidsLI;
		
		
				
		SpinnerNumberModel model = new SpinnerNumberModel(lien.getPoids(), valeurMinJSpinner, valeurMaxJSpinner , pasJSpinner);
		jspinValeur = new JSpinner(model);
		model = new SpinnerNumberModel(pas, valeurMinJSpinner, valeurMaxJSpinner , pasJSpinner);
   ...
   // this.add(new JScrollPane(optionPane));
	...
	// On ajoute le knob
	jknob = new JKnob(this, lien.getPoids()*100/parent.poidsMaxLI);
...
public void propertyChange(PropertyChangeEvent e) {
	...
	else if(e.getSource().equals(jspinValeur))
	{
		...
		if(valeur>=0)
			jknob.valeurModifiee((valeur*100/valeurMax));
		else{
			jknob.valeurModifiee((valeur*100/valeurMin*(-1)));
		}
	}
...
public void actualiserPosition()
{
	...
	// On regarde si le panel dépasse sur les cotés, on ajuste en consequence
	if(this.getX()<0) 
...
public int modifierPoids(int pourcent) {
	...
	if(pourcent>=0){
		jspinValeur.setValue(Math.round((valeurMax*p)));
		resultat = (int)jspinValeur.getValue()*100/parent.poidsMaxLI;
	}
	else{
		jspinValeur.setValue(Math.round(valeurMin*p*(-1)));
		resultat = (int)jspinValeur.getValue()*100/parent.poidsMinLI*(-1);
	}
	return resultat;
}
...
public void stateChanged(ChangeEvent event) {
	...
	if(event.getSource().equals(jspinValeur))
	{
		int valeur = (int)jspinValeur.getValue();
		if(valeur>=0)
			jknob.valeurModifiee((int)jspinValeur.getValue()*100/parent.poidsMaxLI);
		else
			jknob.valeurModifiee((int)jspinValeur.getValue()*100*(-1)/parent.poidsMinLI);
	}
}
