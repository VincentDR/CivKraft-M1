...
public class PanelRatioCognitons extends JJPanel{
	...
	int nCognitons[] = new int[Configuration.getSchemaCognitifEnCourEdition().getCognitons().size()];
	...
	public PanelRatioCognitons(){
		...
		for (int i = 0; i < Configuration.getSchemaCognitifEnCourEdition().getCognitons().size(); i++){
			...
			diag.addItem(Configuration.getSchemaCognitifEnCourEdition().getCognitons().get(i).getType().getCouleur(), Configuration.getSchemaCognitifEnCourEdition().getCognitons().get(i).getNom(), 20.);
	...
	public void actualiser() {
		...
		for (int i = 0; i < tortues.length; i++){
			if (tortues[i].getClass() == Humain.class){
				...
				ArrayList<Cogniton> listeCognitons= h.getEsprit().getCognitons();
		...
		for (int i = 0; i < diagrammes.size(); i++){
			...
			double d;
			if (decompteCognitons.get(Configuration.getSchemaCognitifEnCourEdition().getCognitons().get(i).getNom()) == null){
				d = 0;
				System.out.println("d est nul : " + d + " " + Configuration.getSchemaCognitifEnCourEdition().getCognitons().get(i).getNom());
			}else{
				d = decompteCognitons.get(Configuration.getSchemaCognitifEnCourEdition().getCognitons().get(i).getNom());
				System.out.println("d vaut : " + decompteCognitons.get(Configuration.getSchemaCognitifEnCourEdition().getCognitons().get(i).getNom()));
