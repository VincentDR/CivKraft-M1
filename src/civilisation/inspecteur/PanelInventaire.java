...
public class PanelInventaire extends JPanel{
	...
	JLabel titre = new JLabel(I18nList.CheckLang("Inventaire"));
	...
	public PanelInventaire()
	{
        ...
        String[] entetes = {I18nList.CheckLang("Objet"),I18nList.CheckLang("Nombre")};
	...
	public void actualiser(Turtle t)
	{
		...
		{
			int j = 0;
			while(j < donnees.length && donnees[j][0] != null && !donnees[j][0].equals(allObjets.get(i).getNom()))
			{
				++j;
			}
			if(j < donnees.length)
			{
				...
				{
					donnees[j][0] = allObjets.get(i).getNom();
					donnees[j][1] = 1;
				}
			}
