...
public class FabriqueScheme {
	...
	public static void sauverSchemaCognitif(SchemaCognitif sc, String path){
		...
		for (int i = 0; i < sc.getAttributesNames().size();i++){
			...
			File attributes = new File(dossierAttributes.getPath()+"/"+sc.getAttributesNames().get(i)+Configuration.getExtension());
			try {
		...
		for (int i = 0; i < sc.getConstantsNames().size();i++){
			...
			File constants = new File(dossierConstants.getPath()+"/"+sc.getConstantsNames().get(i)+Configuration.getExtension());
			try {
		...
		for (int i = 0; i < sc.getCognitons().size();i++){
			...
			File cognitons = new File(dossierCognitons.getPath()+"/"+sc.getCognitons().get(i).getNom()+Configuration.getExtension());
			try {
		...
		for (int i = 0; i < sc.getCloudCognitons().size();i++){
			File cloudCognitons = new File(dossierCloudCognitons.getPath()+"/"+sc.getCloudCognitons().get(i).getNom()+Configuration.getExtension());		
			try {
		...
		for (int i = 0; i < sc.getGroups().size();i++){
			File groups = new File(dossierGroups.getPath()+"/"+sc.getGroups().get(i).getName()+Configuration.getExtension());			
			try {
		...
		for (int i = 0; i < sc.getPlans().size();i++){
			...
			File plans = new File(dossierPlans.getPath()+"/"+sc.getPlans().get(i).getNom()+Configuration.getExtension());
			try {
		...
		try {
			out = new PrintWriter(new FileWriter(supp.getPath() + "/" + sc.getNom()+Configuration.getExtension()));
