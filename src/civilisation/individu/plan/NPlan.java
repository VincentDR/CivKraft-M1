...
public class NPlan implements Cloneable, Serializable{
	...
	public NPlan clone() throws CloneNotSupportedException {
	    NPlan nplan = new NPlan();

	    nplan = (NPlan) super.clone();
	    nplan.setActions(new ArrayList<Action>());
	    if(this.actions != null && this.actions.size() > 0){
	    	int incomprehension = this.actions.size();
	    	for(int i = 0; i < incomprehension; i++){
	      		nplan.addAction(this.actions.get(i));
	      	}
	    }
...
//		    System.out.println("plan lance : " + nom);
	...
	public void ecrireAction(PrintWriter out, int iteration, Action a){
		...
		/* Cause les doublons ? à quoi ça sert ?
