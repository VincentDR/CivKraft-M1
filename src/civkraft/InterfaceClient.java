...
public class InterfaceClient extends JFrame implements MouseListener {
	...
	public InterfaceClient(Semaphore s, int width, int height, Client d){
		...
		explications = new JLabel("<html>Recherche de Serveurs Civkraft dans le réseau local ...<br>Vous pouvez lancer un serveur en choisissant l'option héberger partie",JLabel.CENTER);
		Font font = new Font("Arial", Font.BOLD, 12);
		explications.setFont(font);
		explications.setVisible(true);
		
		
		Object[] data = {"Recherche..."};
		...
		envoiScCognitif = new JButton("Envoi Schéma Cognitif");
	...
	public void actualiser(List<AgentAddress> list){
		...
		if(list != null){
			...
			explications.setText("<html>Vous êtes actuellement connecté au serveur civkraft suivant.<br>En attente du lancement de la partie côté serveur...</html>");
			
		}else{
			explications.setText("<html>Rechercher de Serveurs Civkraft dans le réseau local ...<br>Vous pouvez lancer un serveur en choisissant l'option héberger partie");
		}
