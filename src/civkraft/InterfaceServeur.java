...
public class InterfaceServeur extends JFrame implements MouseListener {
	...
	public InterfaceServeur(Semaphore s, int width, int height, Serveur d){
		...
		explications = new JLabel("Serveur démarré avec succès, en attente de joueurs...",JLabel.CENTER);
		Font font = new Font("Arial", Font.BOLD, 12);
		explications.setFont(font);
		explications.setVisible(true);
		
		Object[][] data = {{"Emplacement Vide", "Recherche en cours..."},
	            {"Emplacement Vide", "Recherche en cours..."}};
		String[] columnNames = {"Kernel", "Nom"};
		...
		connection = new JButton("Lancer la partie");		
	...
	public void actualiser(List<AgentAddress> list){
		...
		if(list != null){
			...
			for(int i = 0; i < clientsObtenus.size();i++){
				...
				data[clientsObtenus.size()][0] = "Emplacement Vide";
				data[clientsObtenus.size()][1] = "Recherche en cours...";
			}	
		}
		String[] columnNames = {"Kernel", "Nom"};
		DefaultTableModel model = new DefaultTableModel(data, columnNames);
